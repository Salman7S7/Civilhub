// src/services/bnbcExpertEngine.js
// -----------------------------------------------------------------------------
// CivilHub BNBC 2020 & Bangladesh Civil Engineering Expert Engine (Client-Side)
//
// Governed by:
 ...
 */
export function generateBnbcExpertAnswer(question, context = null) {
  ...
  // 0d. Questions inquiring why AI cannot answer non-civil engineering or live topics
  const isWhyCapability = /(?:why|y)\s+(?:can|cant|cannot|couldnt|wont|didnt)\s+(?:you|u)\b/i.test(cleanLower) ||
                          /why\s+can't\s+you/i.test(cleanLower) ||
                          /why\s+don't\s+you/i.test(cleanLower);
  if (isWhyCapability) {
    return (
      `I am the **CivilHub AI Assistant**, and my core design is specifically focused on the **Bangladesh National Building Code (BNBC 2020)** and municipal construction regulations (such as RAJUK, CDA, KDA, and RDA).\n\n` +
      `Here is why I couldn't provide details on that query:\n` +
      `1. **Domain Specialization**: I am built exclusively for civil engineering, architectural planning, setback rules, and building safety in Bangladesh.\n` +
      `2. **No Real-Time Internet Browsing**: I do not track live sports scores (like today's cricket match), daily weather, or live news feeds.\n\n` +
      `I am, however, always ready to assist you with any civil engineering or construction queries, including:\n` +
      `- Permissible stories for your road width\n` +
      `- Setback requirements, FAR, and Ground Coverage (MGC)\n` +
      `- Geotechnical soil testing & borehole SPT requirements\n` +
      `- Structural design & municipal approval guidelines (RAJUK/CDA/KDA/RDA)`
    );
  }

  // 0e. Off-topic Queries (Cricket, sports, movies, weather, etc.)
  const isOffTopic = /(cricket|football|soccer|score|match|game|sports|player|ipl|bpl|movie|cinema|actor|actress|film|song|music|recipe|cooking|weather|temperature|stock\s*market|crypto|bitcoin|politics|election)/i.test(cleanLower);
  if (isOffTopic && !cleanLower.includes("road") && !cleanLower.includes("building") && !cleanLower.includes("stories") && !cleanLower.includes("setback") && !cleanLower.includes("katha") && !cleanLower.includes("soil")) {
    return (
      `I am the **CivilHub AI Assistant**, and I specialize exclusively in the **Bangladesh National Building Code (BNBC 2020)** and urban construction regulations (RAJUK, CDA, KDA, RDA).\n\n` +
      `Because my design is dedicated to civil engineering, architectural planning, and structural safety, I don't track live sports (like today's cricket match), entertainment, or general news.\n\n` +
      `However, I can gladly help you with any construction questions in Bangladesh, such as:\n` +
      `- Permissible stories on your road width (e.g., 20ft road under RAJUK)\n` +
      `- Setback requirements and Floor Area Ratio (FAR)\n` +
      `- Geotechnical soil testing & borehole SPT requirements (BNBC Part 6)\n` +
      `- Building plan approvals and municipal guidelines`
    );
  }
